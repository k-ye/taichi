...
class TaskCodegen : public IRVisitor {
 ...
 public:
  ...
  void visit(GlobalStoreStmt *stmt) override {
    ...
    const auto dt = stmt->data->element_type();
    emit("{} = {};", at_buffer(stmt->ptr, dt),
         store_as_int_bits(stmt->data->raw_name(), dt));
  }

  void visit(GlobalLoadStmt *stmt) override {
    TI_ASSERT(stmt->width() == 1);
    auto dt = stmt->element_type();
    const auto loaded_int = at_buffer(stmt->ptr, dt);
...
void lower(Kernel *kernel) {
  ...
  irpass::compile_to_executable(kernel->ir.get(), config,
