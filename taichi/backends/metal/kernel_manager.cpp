...
class RuntimeListOpMtlKernel : public SparseRuntimeMtlKernelBase {
 public:
  ...
  explicit RuntimeListOpMtlKernel(Params &params)
    ...
    TI_DEBUG(
        "Registered RuntimeListOpMtlKernel: name={} num_threads={} "
        "parent_snode={} "
  ...
  CompiledTaichiKernel(Params params)
    ...
    for (const auto &ka : ti_kernel_attribs.mtl_kernels_attribs) {
      ...
      if (ktype == KernelTaskType::listgen) {
        RuntimeListOpMtlKernel::Params kparams;
        ...
        kernel = std::make_unique<RuntimeListOpMtlKernel>(kparams);
 ...
 private:
  void init_runtime(int root_id) {
    ...
    for (int i = 0; i < max_snodes; ++i) {
      ...
      const SNodeDescriptor &sn_desc = iter->second;
      // TODO(k-ye): if (!is_gc_able(sn_desc.snode->type)) {
      if (sn_desc.snode->type != SNodeType::pointer) {
        continue;
      }
  ...
  void blit_buffers_and_sync(
    ...
    if (!buffers_to_blit.empty()) {
      ...
      // synchronize_resource(encoder.get(), runtime_buffer_.get());
      end_encoding(encoder.get());
    }
    // Sync
